...
var Grid = function Grid(canvasElement) {
	...
	this.drawMessage = function (msg) {
		...
		_this.ctx.clearRect(grid.centreSpot().x - 50, grid.centreSpot().y - 50, 550, 50);
...
var update = function update() {
	...
	gamePieces.forEach(function (x) {
		if (x.on) {
			x.clearMe(grid);x.forward();
		}
		if (count % (10 - x.speed) === 0 && x.harmful) {
			x.pulse();
		}
		if (x != gamePieces[0] && gamePieces[0].hasCollided(x)) {
			...
			if (x.harmful) {
				...
				grid.clearMe();
				setTimeout(function () {
					startGame();canvas.style["background-color"] = "black";
				}, 3000);
...
var mute = function mute() {
	return gamePieces.forEach(function (x) {
		return x.gainNode.disconnect(audioCtx.destination);
	});
};
var unmute = function unmute() {
	return gamePieces.forEach(function (x) {
		return x.gainNode.connect(audioCtx.destination);
	});
};
